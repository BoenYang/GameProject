...
void MenuScene::addButton()
{
	...
	about->setPosition(ccp(screenWidth - 40, screenHeight - 40));
...
void MenuScene::keyBackClicked(void)
{
	...
	JniMethodInfo minfo;                         //定义Jni函数信息结构体 //getStaticMethodInfo 次函数返回一个bool值表示是否找到此函数
	bool isHave = JniHelper::getStaticMethodInfo(minfo,"com/topteam/app/GameProject", "show", "()V");
